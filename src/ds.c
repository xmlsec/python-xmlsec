...
static PyObject* PyXmlSec_SignatureContext__new__(PyTypeObject *type, PyObject *args, PyObject *kwds) {
    PyXmlSec_SignatureContext* ctx = (PyXmlSec_SignatureContext*)PyType_GenericNew(type, args, kwds);
...
static int PyXmlSec_SignatureContext__init__(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    PyXmlSec_KeysManager* mgr = NULL;

    PYXMLSEC_DEBUGF("%p: init sign context", self);
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "|O&:load_cert_from_file", kwlist, PyXmlSec_KeysManagerConvert, &mgr)) {
        return -1;
    }

    xmlSecDSigCtxPtr handle = xmlSecDSigCtxCreate(mgr != NULL ? mgr->handle : NULL);
    if (handle == NULL) {
        PyXmlSec_SetLastError("failed to create the digital signature context");
        return -1;
    }
    ((PyXmlSec_SignatureContext*)self)->handle = handle;
    return 0;
}
...
static int PyXmlSec_SignatureContextKeySet(PyObject* self, PyObject* value, void* closure) {
    ...
    ctx->handle->signKey = xmlSecKeyDuplicate(keyHandle);
    if (ctx->handle->signKey == NULL) {
...
static PyObject* PyXmlSec_SignatureContextRegisterId(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    if (tmpAttr != attr) {
        if (tmpAttr != NULL) {
            PyErr_SetString(PyXmlSec_Error, "duplicate id.");
            goto ON_FAIL;
        }
...
static PyObject* PyXmlSec_SignatureContextVerify(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    if (handle->status != xmlSecDSigStatusSucceeded) {
        PyErr_SetString(PyXmlSec_VerificationError, "Signature verification failed");
        goto ON_FAIL;
    }
...
static PyObject* PyXmlSec_SignatureContextSignBinary(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    PYXMLSEC_DEBUGF("%p: sign binary - start", self);
...
static PyObject* PyXmlSec_SignatureContextVerifyBinary(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    if (rv < 0 || ctx->signMethod->status != xmlSecTransformStatusOk) {
...
static PyObject* PyXmlSec_SignatureContextSetEnabledKeyData(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    iter = PyObject_GetIter(keydata_list);
    if (iter == NULL) {
        goto ON_FAIL;
    }
