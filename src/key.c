...
static PyObject* PyXmlSec_Key__new__(PyTypeObject *type, PyObject *args, PyObject *kwds) {
    PyXmlSec_Key* key = (PyXmlSec_Key*)PyType_GenericNew(type, args, kwds);
...
static PyObject* PyXmlSec_KeyFromMemory(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OI|z:from_memory", kwlist, &dataObj, &format, &password)) {
        goto ON_FAIL;
    }
    // TODO move this to *_from_file method
    if (PyObject_HasAttrString(dataObj, "read")) {
        dataObj = tmp = PyObject_CallMethod(dataObj, "read", NULL);
    }

    if (dataObj == NULL) goto ON_FAIL;

    Py_ssize_t data_size = 0;
    const char* data;
    if ((data = PyXmlSec_GetAsStringAndSize(dataObj, &data_size)) == NULL) goto ON_FAIL;

    if ((key = PyXmlSec_NewKey1((PyTypeObject*)self)) == NULL) goto ON_FAIL;

    Py_BEGIN_ALLOW_THREADS;
    key->handle = xmlSecCryptoAppKeyLoadMemory((const xmlSecByte*)data, data_size, format, password, NULL, NULL);
    Py_END_ALLOW_THREADS;
...
static PyObject* PyXmlSec_KeyFromFile(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O&H|z:from_file", kwlist,  PyString_FSConverter, &filepath, &format, &password)) {
        goto ON_FAIL;
    }

    const char* filename = PyBytes_AS_STRING(filepath);
    if (filename == NULL) goto ON_FAIL;
    if ((key = PyXmlSec_NewKey1((PyTypeObject*)self)) == NULL) goto ON_FAIL;

    PYXMLSEC_DEBUGF("filename is %s", filename);

    Py_BEGIN_ALLOW_THREADS;
    key->handle = xmlSecCryptoAppKeyLoad(filename, format, password, NULL, NULL);
    Py_END_ALLOW_THREADS;
...
static PyObject* PyXmlSec_KeyGenerate(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O!HI:from_file", kwlist, PyXmlSec_KeyDataType, &keydata, &keysize, &keytype)) {
        goto ON_FAIL;
    }
...
static PyObject* PyXmlSec_KeyCertFromMemory(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OI:load_cert_from_memory", kwlist, &dataObj, &format)) {
        goto ON_FAIL;
    }

    // TODO move to read_from_file
    if (PyObject_HasAttrString(dataObj, "read")) {
        dataObj = tmp = PyObject_CallMethod(dataObj, "read", NULL);
    }
    ...
    int rv = 0;
    Py_BEGIN_ALLOW_THREADS;
    rv = xmlSecCryptoAppKeyCertLoadMemory(((PyXmlSec_Key*)self)->handle, (const xmlSecByte*)data, data_size, format);
    Py_END_ALLOW_THREADS;
    if (rv < 0) {
...
static PyObject* PyXmlSec_KeyCertFromFile(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    PYXMLSEC_DEBUGF("%p: load certificate from memory - start", self);
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O&I:load_cert_from_file", kwlist,  PyString_FSConverter, &filepath, &format)) {
        goto ON_FAIL;
    }
    ...
    int rv = 0;
    Py_BEGIN_ALLOW_THREADS;
    rv = xmlSecCryptoAppKeyCertLoad(((PyXmlSec_Key*)self)->handle, filename, format);
    Py_END_ALLOW_THREADS;
    if (rv < 0) {
        PyXmlSec_SetLastError("cannot load cert");
        goto ON_FAIL;
...
static int PyXmlSec_KeyNameSet(PyObject* self, PyObject* value, void* closure) {
    PYXMLSEC_DEBUGF("%p: set name of key", self);
    ...
    const char* name;
    if ((name = PyXmlSec_GetAsString(value)) == NULL) return -1;
...
static PyObject* PyXmlSec_KeysManager__new__(PyTypeObject *type, PyObject *args, PyObject *kwds) {
    PyXmlSec_KeysManager* mgr = (PyXmlSec_KeysManager*)PyType_GenericNew(type, args, kwds);
...
static PyObject* PyXmlSec_KeysManagerLoadCertFromMemory(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    int rv;
    Py_BEGIN_ALLOW_THREADS;
    rv = xmlSecCryptoAppKeysMngrCertLoadMemory(
        ((PyXmlSec_KeysManager*)self)->handle, (const xmlSecByte*)data, data_size, format, type
    );
    Py_END_ALLOW_THREADS;
    if (rv < 0) {
