...
static PyObject* PyXmlSec_EncryptionContext__new__(PyTypeObject *type, PyObject *args, PyObject *kwds) {
    PyXmlSec_EncryptionContext* ctx = (PyXmlSec_EncryptionContext*)PyType_GenericNew(type, args, kwds);
...
static int PyXmlSec_EncryptionContext__init__(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    xmlSecEncCtxPtr handle = xmlSecEncCtxCreate(mgr != NULL ? mgr->handle : NULL);
    if (handle == NULL) {
        PyXmlSec_SetLastError("failed to create the digital signature context");
        return -1;
    }
    ((PyXmlSec_EncryptionContext*)self)->handle = handle;
    return 0;
}
...
static PyObject* PyXmlSec_EncryptionContextDecrypt(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    if (parent != NULL) {
        ...
        int typeContent = (ttype == NULL || !xmlStrEqual(ttype, xmlSecTypeEncContent));
        xmlFree(ttype);
